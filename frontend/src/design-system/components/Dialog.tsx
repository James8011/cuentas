    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      ...
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            'fixed left-1/2 top-1/2 w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl outline-none',
            nested ? 'z-[120]' : 'z-[100]',
            className,
          )}
        >
          ...
          {children ? <div className="mb-4 space-y-3">{children}</div> : null}
          {footer ? (
